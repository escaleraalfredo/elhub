...
export default function EncuestasPage() {
  ...
  const openComments = (pollId: number) => {
    router.push(`/comunidad/encuestas/${pollId}`);
  };
  ...
  return (
    <div className="min-h-screen bg-dark-bg pb-12">
          ...
          {polls.map((poll) => (
            ...
            >
              <div className="p-6">
                ...
                <h3 className="text-[17px] font-semibold leading-tight mb-6">
                  {poll.question}
                </h3>
                ...
                <div className="space-y-3 mb-6">
                  {poll.options.map((option) => (
                    <button
                      ...
                      disabled={poll.voted}
                      className={`w-full relative overflow-hidden rounded-2xl border transition-all text-left p-4 ${
                        poll.voted 
                          ? poll.userVote === option.id 
                            ? "border-emerald-500 bg-emerald-950/30" 
                            : "border-zinc-700" 
                          : "border-zinc-700 hover:border-pr-red active:scale-[0.985]"
                      }`}
                    >
                      ...
                      <div className="absolute bottom-0 left-0 h-1 bg-zinc-800 w-full">
                        <div 
                          className={`h-1 transition-all duration-300 ${poll.userVote === option.id ? "bg-emerald-500" : "bg-zinc-700"}`}
              ...
              <div className="border-t border-zinc-800 px-6 py-4 flex items-center justify-between text-sm">
                <button
                  ...
                  className={`flex items-center gap-2 transition-all ${poll.liked ? "text-red-500" : "text-zinc-400 hover:text-red-500"}`}
                >
                  <Heart className={`w-5 h-5 ${poll.liked ? "fill-current" : ""}`} />
                  <span>{poll.likes}</span>
                </button>

                <button
                  onClick={() => openComments(poll.id)}
                ...
                <div className="text-xs text-zinc-500">
                  {poll.voted ? "Ya votaste" : "Vota ahora"}
                </div>
              </div>
            </div>
          ))}
      ...
      {/* Floating + Button - same as Temas page */}
      <button
          ...
          <div className="bg-zinc-900 w-full max-w-md mx-auto rounded-t-3xl md:rounded-3xl overflow-hidden">
            <div className="p-6">
              ...
              <textarea
                ...
                className="w-full h-24 bg-zinc-800 border border-zinc-700 rounded-2xl px-4 py-3 text-white placeholder-zinc-500 focus:outline-none focus:border-pr-red resize-y"
              />
              ...
              <p className="text-center text-xs text-zinc-500 mt-4">
                Recibirás +15 puntos por crear una encuesta
              </p>
            </div>
          </div>
        </div>
